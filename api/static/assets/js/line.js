class LineVis {
    ...
    wrangleData(){
        ...
        vis.keys.forEach(function (key) {
            let array = Array.from(rollupData.get(key), ([key, value]) => ({
                timestamp: key,
                value: value
            }));
            array.sort((a,b) => a.timestamp - b.timestamp);

            vis.data.push({category: key, dates: array});
        });
