class LineVis {
    ...
    wrangleData(){
        ...
        vis.data = d3.rollup(vis.data, v => v.length, d => d.Category, d => d.Timestamp)
    ...
    updateVis(){
        ...
        let lineInside = d3.line()
            .x(d => vis.x(d3.map(d, x=>x)[0]))
            .y(d => vis.y(d3.map(d, x=>x)[1]))
